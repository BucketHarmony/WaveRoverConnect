...
### 1. Router/Firewall Interference

**Problem:** Some routers (especially mesh networks like eero) intercept HTTP requests.

**Symptoms:**
- Commands seem to send but rover doesn't respond
- Response body contains HTML instead of JSON
- Response mentions content filtering or security

**Solution:**
- Disable content filtering/security features
- Use the rover's AP mode directly for testing
- Check response body to verify it's JSON

**Detection code:**
```python
response = requests.get(url, timeout=2.0)
if response.text.strip().startswith('<'):
    print("WARNING: Got HTML response - router may be intercepting")
```

### 2. Turns Not Working

**Problem:** Spin or arc commands sent but rover doesn't turn.

**Cause:** Speed too low to overcome friction.

**Solution:** Increase turn speeds:
```python
# Instead of:
{"T": 1, "L": 0.15, "R": -0.15}  # Too slow, won't turn

# Use:
{"T": 1, "L": 0.35, "R": -0.35}  # Will turn reliably
```

### 3. Motion Stops Unexpectedly

**Problem:** Rover stops moving after a few seconds.

**Cause:** Safety timeout - no keepalive commands being sent.

**Solution:** Implement keepalive loop (see above).

### 4. Connection Timeouts

**Problem:** Frequent connection timeouts or failures.

**Solutions:**
- Use shorter timeout (2s is usually enough)
- Implement retry logic (2-3 retries)
- Check WiFi signal strength (use T:405)
- Move closer to router/rover

### 5. Commands Queuing Up

**Problem:** Rover seems to execute old commands.

**Cause:** Network latency or command buffering.

**Solution:**
- Always send stop command explicitly
